		...
		options: &sdk.OperationInfo{
			...
			Input: map[string]*sdkcore.AutoFormSchema{
				"from_path": autoform.NewShortTextField().
					...
					SetDescription("The current path of the file (e.g. /folder1/oldfile.txt").
					SetRequired(true).Build(),
				"to_path": autoform.NewShortTextField().
					SetDisplayName("To Path").
					SetDescription("The new path for the file (e.g. /folder2/newfile.txt").
					SetRequired(true).Build(),
...
func (c *MoveFileOperation) Run(ctx *sdk.RunContext) (sdk2.JSON, error) {
...
func (c *MoveFileOperation) Test(ctx *sdk.RunContext) (sdk2.JSON, error) {
	return c.Run(ctx)
}
