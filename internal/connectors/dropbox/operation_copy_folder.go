		...
		options: &sdk.OperationInfo{
			...
			Input: map[string]*sdkcore.AutoFormSchema{
				"from_path": autoform.NewShortTextField().
					...
					SetDescription("The source path of the folder (e.g. /folder1/sourceFolder)").
					SetRequired(true).Build(),
				"to_path": autoform.NewShortTextField().
					SetDisplayName("To Path").
					SetDescription("The destination path for the copied folder (e.g. /folder2/destinationFolder)").
					SetRequired(true).Build(),
...
func (c *CopyFolderOperation) Run(ctx *sdk.RunContext) (sdk2.JSON, error) {
...
func (c *CopyFolderOperation) Test(ctx *sdk.RunContext) (sdk2.JSON, error) {
	return c.Run(ctx)
}
