...
func NewGetTeamsOperation() *GetTeamsOperation {
	return &GetTeamsOperation{
		options: &sdk.OperationInfo{
			...
			Description: "Get list of authorized teams(Workspace)",
			RequireAuth: true,
			Auth:        sharedAuth,
...
func (c *GetTeamsOperation) Run(ctx *sdk.RunContext) (sdk2.JSON, error) {
	if ctx.Auth.AccessToken == "" {
		return nil, errors.New("missing clickup auth token")
	}
	accessToken := ctx.Auth.AccessToken

	teams, err := getTeams(accessToken)
	if err != nil {
		return nil, err
	}
...
func (c *GetTeamsOperation) Test(ctx *sdk.RunContext) (sdk2.JSON, error) {
	return c.Run(ctx)
}
