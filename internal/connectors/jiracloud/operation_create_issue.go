...
func (c *CreateIssueOperation) Run(ctx *sdk.RunContext) (sdk2.JSON, error) {
	input := sdk.InputToType[createIssueOperationProps](ctx)
	...
	reqURL := instanceURL
	resp, err := jiraRequest(ctx.Auth.Extra["email"], ctx.Auth.Extra["api-token"], reqURL, http.MethodPost, "", data)
	if err != nil {
		return nil, err
	}
...
func (c *CreateIssueOperation) Test(ctx *sdk.RunContext) (sdk2.JSON, error) {
	return c.Run(ctx)
}
