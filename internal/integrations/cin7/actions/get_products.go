...
func (a *GetProductsAction) Properties() *smartform.FormSchema {
	form := smartform.NewForm("get_products", "Get Products")

	form.NumberField("page-limit", "Page limit").
		Required(false).
		HelpText("Specifies the page limit for products.")
...
func (a *GetProductsAction) Perform(ctx sdkcontext.PerformContext) (core.JSON, error) {
	...
	input, err := sdk.InputToTypeSafely[getProductsActionProps](ctx)
	if err != nil {
		return nil, err
	}
